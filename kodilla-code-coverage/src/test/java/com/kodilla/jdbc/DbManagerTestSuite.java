...
public class DbManagerTestSuite {
    ...
    public void testSelectUsersAndPosts() throws SQLException {
        ...
        String postsCountQuery = "SELECT COUNT(*)\n" +
                "FROM USERS U\n" +
                "JOIN POSTS P ON U.ID = P.USER_ID\n" +
                "GROUP BY P.USER_ID\n" +
                "HAVING COUNT(*) > 1;";
        Statement statement2 = dbManager2.getConnection().createStatement();
        ResultSet rs2 = statement2.executeQuery(postsCountQuery);
        int postsCount = 0;
        while (rs2.next()) {
            postsCount = rs2.getInt("COUNT(*)");
        }
        String postssql = "INSERT INTO POSTS (USER_ID, BODY) VALUES (3, 'Quite a nice place to talk!')";
        statement2.executeUpdate(postssql);
        postssql = "INSERT INTO POSTS (USER_ID, BODY) VALUES (3, 'I think I will stay here :)')";
        statement2.executeUpdate(postssql);
        //When
        String postsQuery = "SELECT U.FIRSTNAME, U.LASTNAME, COUNT(*) AS POSTS_NUMBER\n" +
                "FROM USERS U\n" +
                "JOIN POSTS P ON U.ID = P.USER_ID\n" +
                "GROUP BY P.USER_ID\n" +
                "HAVING COUNT(*) > 1";
        statement2 = dbManager2.getConnection().createStatement();
        rs2 = statement2.executeQuery(postsQuery);
